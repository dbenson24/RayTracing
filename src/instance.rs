...
use bvh::{ray::{IntersectionRay, Ray, Intersection}, aabb::{Bounded, AABB}};
...
impl <T> Instance<T> {
    ...
    pub fn from_t(obj: Arc<T>, translation: Vec3) -> Self {
        let transform = Mat4::from_scale_rotation_translation(Vec3::ONE, Quat::IDENTITY, translation);
        Self {
            obj,
            transform
        }
...
impl <T> IntersectionRay for Instance<T> where T: IntersectionRay {
    fn intersects_ray(&self, ray: &Ray, t_min: bvh::Real, t_max: bvh::Real) -> Option<Intersection> {
        let inv = self.transform.inverse();
        let local_ray = Ray::new(inv.transform_point3(ray.origin), inv.transform_vector3(ray.direction).normalize());
...
impl <T> Bounded for Instance<T> where T: Bounded {
    fn aabb(&self) -> AABB {
